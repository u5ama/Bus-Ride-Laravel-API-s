...
class Driver extends Authenticatable implements JWTSubject
{
    ...
    {
        return $this->hasOne('App\Models\DriverProfile', 'dp_user_id');
    }
    ...
    {
        return $this->hasOne('App\Models\DriverCurrentLocation', 'dcl_user_id');
    }

    // Driver Account Table Relation
    public function DriverAccount()
    {
        return $this->hasMany('App\Models\DriverAccount', 'dc_target_id');
    }

    // Driver Rating Table Relation
    public function DriverRating()
    {
        return $this->hasMany('App\Models\DriverRating', 'dr_driver_id');
    }

    // Drivers with ID Table Relation
    public static function getdriver($id){
        $user = DriverResource::collection(Driver::with('driverProf','DriverCurrentLocation')->where('id',$id)->get());
       if(isset($user[0])){
           $user = $user[0];
       }else{
           $user = [];
    ...
    public static function getdriverfull($id){
        $user = GetMyDriverProfile::collection(Driver::with('driverProf','DriverCurrentLocation')->where('id',$id)->get());
        return $user[0];
    }
}

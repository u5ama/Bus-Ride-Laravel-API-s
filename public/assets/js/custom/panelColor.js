$(function () {
    ...
    const table = $('#data-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: APP_URL + '/panel-color',
            type: 'GET',
        },
        columns: [
            {data: 'id', name: 'id'},
            {data: 'color_key_field', name: 'color_key_field'},
            {data: 'color_key_value', name: 'color_key_value'},
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.success === true) {
                        ...
                        setTimeout(function () {
                            window.location.href = APP_URL + '/panel-color'
                        }, 1000);
    ...
    function deleteRecord(value_id) {
        $.ajax({
            type: 'DELETE',
            url: APP_URL + '/panel-color' + '/' + value_id,
            success: function (data) {
                successToast(data.message, 'success');
                table.draw()
                loaderHide();
    ...
    function changeStatus(value_id, status) {
        $.ajax({
            type: 'GET',
            url: APP_URL + '/panelColorChangeStatus/' + value_id + '/' + status,
            success: function (data) {
                successToast(data.message, 'success');
                table.draw()
                loaderHide();
